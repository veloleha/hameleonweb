...
function initSufler() {
  const panel = el('suflerPanel');
  const btnStart = el('btnStartSufler');
  const btnStop = el('btnStopSufler');
  const urlRow = el('suflerUrlRow');
  const urlInput = el('suflerUrl');
  const btnCopy = el('btnCopySuflerUrl');
  const statusEl = el('suflerStatus');

  if (!panel || !btnStart || !btnStop) return;
  ...
  btnCopy.addEventListener('click', async () => {
    try {
      ...
      urlInput.select();
      setStatus('Ctrl+C для копирования.');
    }
  });

  if (window.api.onRecStarted) {
    window.api.onRecStarted((data) => {
      if (data && data.accountId) showPanel(data.accountId);
    });
  }

  if (window.api.onRecStopped) {
    window.api.onRecStopped(() => {
      hidePanel();
    });
  }
}
